...
const PROTOCOL_STEPS = [
  {
    title: "Welcome",
    detail: "Сбор гостей, первый контакт с площадкой, welcome-drink и мягкий вход в атмосферу вечера.",
  },
  {
    title: "Концерт",
    detail: "Главный лайв-блок со сценой, живой группой, номерами гостей и плотным вниманием зала.",
  },
  {
    title: "Нетворкинг",
    detail: "Пауза для разговоров, знакомств, коротких встреч, обсуждений и естественного продолжения контактов.",
  },
  {
    title: "Afterparty",
    detail: "Финальная часть вечера с более свободным общением, музыкой и закреплением новых связей.",
  },
];
    ...
    >
      ...
      <div className="space-y-4">
        ...
        <div
          {...label.bind}
          style={label.style}
          className="font-display text-2xl font-black uppercase leading-[0.92] tracking-tight text-white sm:text-4xl"
        >
          {label.text}
        </div>
      </div>
      ...
      <div className="pointer-events-none absolute inset-x-4 bottom-4 translate-y-2 border border-zinc-600 bg-zinc-950/95 p-4 opacity-0 shadow-2xl transition-all duration-300 group-hover:translate-y-0 group-hover:opacity-100">
        <div className="mb-2 text-[10px] font-mono font-black uppercase tracking-[0.3em] text-[#00FF41]">Подробнее</div>
...
const EveningProtocol = () => {
  ...
  return (
    <section
      ...
      {...section.bind}
      style={section.style}
      className="w-full border-y border-zinc-800 bg-[#080808] px-6 py-20 sm:px-12 lg:px-20 xl:px-32"
    >
      <div className="mx-auto flex min-h-[50vh] w-full max-w-[2000px] flex-col justify-center">
        <div className="mb-12 max-w-3xl space-y-4">
          <span
            {...eyebrow.bind}
            style={eyebrow.style}
            className="border-l-2 border-[#00FF41] pl-4 text-[10px] font-mono font-black uppercase tracking-[0.5em] text-[#00FF41]"
          >
            {eyebrow.text}
          </span>
          <h2
            {...title.bind}
            style={title.style}
            className="font-display text-4xl font-black uppercase leading-none tracking-tighter text-white sm:text-6xl"
          >
            {title.text}
          </h2>
        </div>
