...
function RockstarsStat({ value, label, index }: { key?: React.Key; value: string; label: string; index: number }) {
  ...
  return (
    <div className="bg-black px-5 py-6 sm:px-7 sm:py-8">
      <div {...statValue.bind} style={statValue.style} className="font-display text-4xl font-black leading-none tracking-tight text-[#BEEB08] sm:text-5xl">
        {statValue.text}
      </div>
      <div {...statLabel.bind} style={statLabel.style} className="mt-3 max-w-[14ch] text-[10px] font-mono font-black uppercase tracking-[0.3em] text-zinc-500">
        {statLabel.text}
      </div>
    </div>
  );
...
function LineupCard({ role, index }: { key?: React.Key; role: string; index: number }) {
  ...
  const slotOverride = useEditorOverrides(`industry-rockstars-card-${index + 1}-slot`, { text: `LINEUP SLOT 0${index + 1}` });
  ...
  return (
    <motion.div
      ...
      {...card.bind}
      style={card.style}
      className="group relative overflow-hidden border border-zinc-700 bg-[linear-gradient(180deg,#111114_0%,#090909_100%)] p-6 transition-colors duration-300 hover:border-[#BEEB08]/50"
    >
      <div className="absolute inset-x-0 top-0 h-1 bg-gradient-to-r from-[#FF007F] via-[#BEEB08] to-transparent opacity-70" />
      <div className="mb-8 flex items-center justify-between gap-4">
        <span {...slotOverride.bind} style={slotOverride.style} className="text-[10px] font-mono font-black uppercase tracking-[0.35em] text-zinc-500">
          {slotOverride.text}
        </span>
        <Mic2 className="h-5 w-5 text-[#FF007F] transition-colors duration-300 group-hover:text-[#BEEB08]" />
      </div>
      <div {...roleOverride.bind} style={roleOverride.style} className="font-display max-w-[12ch] whitespace-pre-line text-3xl font-black uppercase leading-[0.9] tracking-tight text-white">
        {roleOverride.text}
      </div>
    </motion.div>
  );
      ...
      <div className="mx-auto grid w-full max-w-[2000px] grid-cols-1 gap-16 lg:grid-cols-[1.1fr_0.9fr] lg:gap-12 xl:gap-20">
        <div className="space-y-10">
          <div className="space-y-6">
            <span {...eyebrow.bind} style={eyebrow.style} className="inline-block border border-[#FF007F]/25 bg-[#FF007F]/10 px-4 py-2 text-[10px] font-mono font-black uppercase tracking-[0.4em] text-[#FF007F]">
              {eyebrow.text}
            </span>
            <h2 {...title.bind} style={title.style} className="font-display max-w-[10ch] whitespace-pre-line text-5xl font-black uppercase leading-[0.84] tracking-[-0.04em] text-white sm:text-6xl lg:text-7xl xl:text-[5.5rem]">
              {title.text}
            </h2>
          </div>
