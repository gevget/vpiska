...
const IntegrationsBuilder = ({ openModalWithPreset }: IntegrationsBuilderProps) => {
  return (
    <section
      id="integrations-block"
      className="relative w-full border-t border-zinc-800/70 px-6 py-24 sm:px-12 sm:py-32 lg:px-20 xl:px-32 xl:py-40"
    >
      <div className="mx-auto w-full max-w-[2000px] space-y-24">
        <div className="max-w-5xl space-y-10">
          <div className="space-y-4">
            <span className="block text-[10px] font-mono font-black uppercase tracking-[0.4em] text-zinc-500 sm:text-xs">
        ...
        <div className="hide-scrollbar -mx-6 flex overflow-x-auto px-6 pb-8 lg:mx-0 lg:grid lg:grid-cols-3 lg:gap-px lg:border lg:border-zinc-700 lg:bg-zinc-700 xl:grid-cols-4">
          {INTEGRATION_METHODS.map((method) => {
            ...
            return (
              <div
                key={method.title}
                className="group relative flex min-h-[300px] min-w-[280px] flex-col overflow-hidden border border-zinc-700 bg-black p-8 transition-all duration-500 hover:bg-zinc-950 sm:min-w-[340px] sm:p-10 lg:min-w-0 lg:border-none"
              >
                <div className="space-y-8">
                  <div className="flex items-start justify-between">
                    <div className="border border-zinc-700 p-3 text-white transition-all duration-500 group-hover:border-[#00FF41] group-hover:text-[#00FF41]">
                      <Icon className="h-6 w-6 stroke-[1.5]" />
                    </div>

                    {method.tag && (
                      <div
                        className={`px-2 py-0.5 text-[8px] font-mono font-black uppercase tracking-widest border ${
                  ...
                  <div className="space-y-3">
                    <h4 className="text-sm font-mono font-black uppercase leading-tight tracking-wider text-white transition-colors duration-500 group-hover:text-[#00FF41]">
                      {method.title}
                    </h4>
                ...
                <div className="mt-auto pt-8 opacity-0 transition-opacity duration-500 group-hover:opacity-100">
                  <button
                    ...
                    className="flex items-center gap-2 text-[10px] font-mono font-black uppercase tracking-widest text-[#00FF41]"
                  >
                    Запросить →
                  </button>
                </div>
